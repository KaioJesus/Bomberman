...
void PlantaBomba(Game *game)
{
    ...
    if (game->jogador.bombaContador <= 0)
        return;
...
void ExplodeBomba(Game *game, int bombIndex)
{
    ...
    for (int d = 0; d < 4; d++)
    {
        ...
        { // Raio da explosão de 2 blocos
            ...
            if (bloco == 'W')
                break; // Parede indestrutível bloqueia a explosão

            if (bloco == 'D' || bloco == 'B')
            {                               // Bloco destrutível ou bomba
